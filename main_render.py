...
def salvar_imagem_sem_distorcao(binary_content, target_path):
    ...
    try:
        ...
        # Leve realce de contraste e nitidez para estilo cinematográfico
...
def N3_gerar_pollinations(prompt_ingles, target_path):
    ...
    for tentativa in range(2):
        try:
            time.sleep(3)  # Pausa preventiva anti-429
...
def N4_buscar_foto_noticia_real(query_especifico, target_path):
    """Nível 4: Busca de Fotos Jornalísticas Reais em HD via DuckDuckGo"""
    try:
                    ...
                    try:
                        ...
                        if res.status_code == 200 and len(res.content) > 15000:
                            if salvar_imagem_sem_distorcao(res.content, target_path):
                                print("   ✅ [Nível 4] Foto real de alta definição capturada!")
                                return True
                    except Exception:
                        continue
    except Exception as e:
        print(f"   ⚠️ [N4] Erro na busca de notícias reais: {e}")
    return False

def adquirir_imagem_cinematografica(prompt_ia, query_real, gemini_key, hf_token, target_path):
    """Gerenciador garantido em cascata"""
    time.sleep(1.5)  # Pausa de estabilização de rede
    ...
    # 4. Busca Foto Real Específica da Notícia
    if N4_buscar_foto_noticia_real(query_real, target_path):
        return True

    # Fallback extremo com query simplificada
    return N4_buscar_foto_noticia_real("international news summit press conference photo", target_path)
